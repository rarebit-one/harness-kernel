...
export function capabilityStdioConfigFromEnv(): CapabilityStdioConfig {
  const workspaceId = process.env.JUMPDRIVE_WORKSPACE_ID
  const workdir = process.env.JUMPDRIVE_WORKDIR
  const emissionsFile = process.env.JUMPDRIVE_EMISSIONS_FILE
  if (!workspaceId || !workdir || !emissionsFile) {
    throw new Error(
      "JUMPDRIVE_WORKSPACE_ID, JUMPDRIVE_WORKDIR and JUMPDRIVE_EMISSIONS_FILE are required",
    )
  }
