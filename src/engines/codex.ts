...
export class CodexEngine implements AgentEngine {
  ...
  async run(spec: RunSpec, ctx: EngineContext): Promise<EngineResult> {
    ...
    const emissionsDir = await mkdtemp(path.join(tmpdir(), "jumpdrive-emit-"))
    ...
    const codexHome = await mkdtemp(path.join(tmpdir(), "jumpdrive-codex-home-"))
    ...
    const capabilityEnv: Record<string, string> = {
      JUMPDRIVE_WORKSPACE_ID: spec.workspaceId,
      JUMPDRIVE_WORKDIR: spec.workdir,
      JUMPDRIVE_EMISSIONS_FILE: emissionsFile,
    }
...
function buildPrompt(spec: RunSpec): string {
  ...
  return [
    ...
    "```",
    "",
    "You are running as a Jumpdrive workflow. The current working directory is the",
