// The shapes a caller hands the kernel. Structurally identical to the five
// types the extracted modules previously imported from jumpdrive-runner's
// `types.ts` — the runner will re-export them from here once it migrates onto
// this package, so the wire contract it speaks with its control plane is
// unaffected. Nothing here is application-specific: these describe what a run
// is *given* (connectors, permissions, a workflow) and what it can *emit*
// (knowledge, issues), not any particular product's domain.
