...
/**
 * The client identity announced to every MCP server we connect to, suffixed
 * with the connector's own name. Servers use it for logging and telemetry.
 */
...
export async function connectWithTransport(
  ...
  const client = new Client({ name: `${MCP_CLIENT_NAME}/${name}`, version: "0.1.0" })
