 ...
 * self-documents what a real run WOULD have used. That makes model/key resolution
 * OBSERVABLE offline — the e2e harness reads it back out of the run's output
 * artifact to assert the control plane's precedence
 * (rarebit-one/jumpdrive-web#204). It never echoes a key, only its last-4 marker.
 */
export class MockProvider implements Provider {
  ...
  // "What a real provider would have used" for the model: the resolved model from
  // the payload (workflow YAML → org default, control-plane side), else the
  // runner's env-default model (the SAME env vars the real providers read), else
  ...
  complete(req: CompletionRequest): Promise<string> {
    ...
    return Promise.resolve(
      [
        ...
        "",
        "_Generated offline by the Jumpdrive mock provider. Set ANTHROPIC_API_KEY",
