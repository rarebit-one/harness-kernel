...
v0 is a **verbatim extraction** of the already-decoupled core of
[`jumpdrive-runner`](https://github.com/rarebit-one/jumpdrive-runner). Behavior
is intentionally identical to that runner's — that is the whole point of the v0
lift, and the runner will be migrated onto this package as its first consumer.
...
- **Kernel minimality is the design constraint.** The name is the forcing
  ...
  perception routing, a control plane, a product's data model — belongs in that
  application's own harness layer, not here. When you feel the urge to add it
  here, that urge is the signal it goes elsewhere.
